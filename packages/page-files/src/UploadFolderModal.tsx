...
import { SaveFile, UploadRes, NormalizedFile } from './types';

export interface Props {
  folderFiles: NormalizedFile[],
  onClose?: () => void,
...
function UploadFolderModal (p: Props): React.ReactElement<Props> {
  const { folderFiles, onClose = NOOP, onSuccess = NOOP } = p;
  ...
  const folderName = folderFiles[0].path.split('/')[0];
  const folderSize = folderFiles.reduce((prev, { size }) => prev + size, 0);
  const fileSizeError = folderSize > 100 * 1024 * 1024;
  const [error, setError] = useState('');
  const errorText = fileSizeError ? t<string>('Do not upload folders larger than 100MB!') : error;
  ...
  const _onClickUp = useCallback(async () => {
    ...
    try {
      ...
      form.append('file', file, folderName);
